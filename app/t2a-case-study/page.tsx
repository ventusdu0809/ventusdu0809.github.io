  ...
  return (
    <main className="t2a-page">
      ...
      <section className="t2a-program-hero t2a-shell" id="overview">
        <div>
          <p className="eyebrow">T2A 音效生成评测</p>
          <h1>音效生成评测<br />音质与文本符合度</h1>
          <p className="t2a-program-subtitle">两阶段听评：建立评分规则，再比较 Stable Audio Open 1.0 与 Stable Audio 3 Medium。</p>
          <p className="t2a-lead">本人完成正式听评、复测和失败案例裁决；分别记录音质、文本符合度与具体错误。</p>
      ...
      <section className="t2a-section t2a-section-tint" id="cases">
        <div className="t2a-shell">
          <header className="t2a-section-heading"><p>先听样本</p><h2>试听样本与评分</h2><p>四条真实音频，分别展示正向参照、方向、数量和声源问题。波形只表示振幅，不代替听评。</p></header>
          <div className="t2a-audio-grid">
            {cases.map((item) => <article key={item.id}><span>{item.id}</span><h3>{item.title}</h3><strong className="t2a-scoreline">{item.score} · {item.statusLabel}（<code>{item.statusCode}</code>）</strong><p>{item.note}</p><img className="sample-wave" src={"/images/portfolio/" + item.audio.split("/").pop()?.replace(".mp3",".png")} alt={item.id + " 实际音频振幅波形，仅辅助浏览，不代表语义判定"} width="1000" height="220" loading="lazy"/><audio controls preload="none" aria-label={`${item.id} ${item.title}试听音频`}><source src={item.audio} type="audio/mpeg" />你的浏览器不支持音频播放。</audio></article>)}
          </div>
        </div>
      </section>
      ...
      <section className="t2a-section t2a-section-tint" id="protocol">
        <div className="t2a-shell">
          <header className="t2a-section-heading"><p>02 / 盲听流程</p><h2>先盲听，再阅读 Prompt</h2><p>正式听评在安静室内完成。系统音量保持不变，但未进行标准化声压级校准。</p></header>
      ...
      <section className="t2a-section t2a-shell" id="results">
        <header className="t2a-section-heading"><p>03 / 受控比较</p><h2>当前测试集未观察到明确的总体优势方向</h2><p>{evaluation.phase2.conclusion}</p></header>
      ...
      <section className="t2a-section t2a-section-tint" id="badcase">
        <div className="t2a-shell">
          <header className="t2a-section-heading"><p>04 / 失败类型</p><h2>按类型记录生成错误</h2><p>以下观察描述当前输出行为，不归因于训练数据、模型结构或采样机制。</p></header>
      ...
      <section className="t2a-section t2a-shell" id="repeat">
        <header className="t2a-section-heading"><p>05 / 隐藏重复</p><h2>隐藏重复检查同一评测人的复测稳定性</h2><p>第一阶段加入 20 条隐藏重复；第二阶段加入 40 条重复样本，与对应原样本构成 40 对并用于主分析。两阶段隐藏重复均不进入正式样本统计。</p></header>
      ...
      <section className="t2a-section t2a-shell t2a-details-section" aria-labelledby="details-title">
        <header className="t2a-section-heading"><p>补充材料</p><h2 id="details-title">方法与版本材料</h2></header>
        ...
        <details className="t2a-collapsible">
          ...
          <div><p>早期 Acoustic Diagnostics（声学诊断）从文件、频谱、空间与能量等角度检查解码、声道和响度风险。客观指标只用于定位与复听，不替代人工语义裁决。</p><p>CLAP 仅作前 5 秒工作流的边界检查，未显示与人工评分的有效单调关系。完整七类 FAD/JS 保持 NOT RUN。</p><p><Link href="/point-line-scene-framework#evaluation-system">查看声学诊断与主观听评的分工 →</Link></p></div>
        </details>
        <details className="t2a-collapsible">
